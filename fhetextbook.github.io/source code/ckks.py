...
def GenerateParam():
    ...
    if is_random_param:
        ...
        if len(ckks_vector)*2 != N or len(ckks_vector2)*2 != N:
            print("Error: The dimensions of the input vectors are not " + str(N) +
                  ", but " + str(len(ckks_vector)) + ", " + str(len(ckks_vector2)))
            sys.exit()
    if M <= 0:
...
# '''
    ...
    print("Selected primitive " + str(M) +
          "-th root of unity: " + str(primitive_root_of_unity))
    print("- " + str(N) + " roots of the " + str(M) +
          '-th cyclotomic polynomial : ' + str(root_values))
    print()
    for root in root_values:
        ...
        print('- Roots of the ' + str(M) + '-th cyclotomic polynomial generated by the co-prime-' + str(coprimes) +
              '-exponented primitive ' + str(M) + '-th root of unity ' + str(root) + ' : ' + str(generated_roots))
    print()
    for root in root_values:
        generated_roots = []
        exponents = []
        for exp in range(M):
            ...
            generated_roots.append((int(root) ** exp))
        print('- All ' + str(M) + '-th roots of unity generated by the ' + str(exponents) +
              '-exponented primitive ' + str(M) + '-th root of unity ' + str(root) + ' : ' + str(generated_roots))
    print()
    ...
    if is_random_param:
        ...
        ckks_vector2 = []
        for i in range(N//2):
            ckks_vector.append(random.uniform(1.0, 10.0) +
                               1j * random.uniform(1.0, 10.0))
            ckks_vector2.append(random.uniform(1.0, 10.0) +
                                1j * random.uniform(1.0, 10.0))
...
class CKKSEncoder:
    ...
    def vandermonde(M: int) -> np.array:
        ...
        for h in range(int(N/2)):
            ...
            if new_root in cyclic_x_values_ordered:
                print("Error 1: root " + str(new_root) +
                      " is already in " + str(cyclic_x_values_ordered))
                sys.exit(0)
        ...
        for h in reversed(range(int(N/2))):
            ...
            if new_root in cyclic_x_values_ordered:
                print("Error 2: root " + str(new_root) +
                      " is already in " + str(cyclic_x_values_ordered))
                print("N: " + str(N))
                sys.exit(0)
            cyclic_x_values_ordered.append(new_root)
    ...
    def create_sigma_R_basis(self):
        ...
        self.sigma_R_basis = np.array(
            np.array(self.vandermonde(self.M)).T, dtype=object)
...
def encode(self, input_vector: np.array) -> Polynomial:
    ...
    for i in range(N // 2):
        ...
        # print("Appended")
    # print(input_vector)
    basis_coordinates = np.matmul(
        N_inverse * self.sigma_R_basis, anti_diagonal_matrix).dot(input_vector)
...
def main():
    ...
    # divide by a redundant scale factor
    p3_rotated = rotate_homomorphic(p3_rotated, rotation_offset, N)
    ...
    print('after polynomial rotation by ' + str(rotation_offset) +
          ' positions and reduction of 1 + 2: ' + str(p3_rotated))
    print()
    # print('before polynomial reduction of 1 * 2: ' + str(p * p2))
    ...
    print('rotated decodec vector 1 + 2 by ' + str(rotation_offset) +
          ' positions: ' + str(decoded_z3_rotated))
    print()
    ...
    for i in range(N//2):
        ...
        print("[FINAL DECODED RESULT " + str(test_index) + "] : " +
              str(z3) + " == " + str(decoded_z3) + " <---- ALMOST CORRECT")
        print()
